...
local function setrank(msg, name, value) -- setrank function
  ...
  if hash then
	...
	return send_msg('chat#id'..msg.to.id, '„ﬁ«„ ò«—»— ('..name..') »Â '..value..'  €??— œ«œÂ ‘œ ', ok_cb,  true)
  end
end
local function res_user_callback(extra, success, result) -- /info <username> function
   ...
   Username = '‰œ«—œ'
    ...
    if not value then
	 if result.id == tonumber(Arian) then
	   text = text..'„ﬁ«„ : „œ?— ò· —»«  (Executive Admin) \n\n'
	  elseif is_admin2(result.id) then
	   text = text..'„ﬁ«„ : «œ„?‰ —»«  (Admin) \n\n'
	  elseif is_owner2(result.id, extra.chat2) then
	   text = text..'„ﬁ«„ : „œ?— ò· ê—ÊÂ (Owner) \n\n'
	  elseif is_momod2(result.id, extra.chat2) then
	    text = text..'„ﬁ«„ : „œ?— ê—ÊÂ (Moderator) \n\n'
      else
  ...
  text = text..' ⁄œ«œ Å?«„ Â«? ›—” «œÂ ‘œÂ : '..user_info_msgs..'\n\n'
  text = text..'@PersianGulf_Team: ?„ Å—‘?‰ êÊ·›'
  send_msg(extra.receiver, text, ok_cb,  true)
  else
	send_msg(extra.receiver, extra.user..' ‰«„ ò«—»—? „Ê—œ ‰Ÿ— ?«›  ‰‘œ.', ok_cb, false)
  end
end
...
local function action_by_id(extra, success, result)  -- /info <ID> function
 ...
 if result.username then
  ...
  send_msg(extra.receiver, '«?œ? ‘Œ’ „Ê—œ ‰Ÿ— œ— ”?” „ À»  ‰‘œÂ «” .\n«“ œ” Ê— “?— «” ›«œÂ ò‰?œ\n/info @username', ok_cb, false)
  end
end
...
local function run(msg, matches)
 ...
 if matches[1]:lower() == 'info' and not matches[2] then
  ...
  if msg.from.username then
   ...
   local text = text..'›«„?· : '..(msg.from.last_name or '‰œ«—œ')..'\n'	
   local text = text..'?Ê“— : '..Username..'\n'
   local text = text..'«?œ? ò«—»—? : '..msg.from.id..'\n\n'
   local hash = 'rank:'..msg.to.id..':variables'
	if hash then
	 ...
	 local uhash = 'user:'..msg.from.id
      local text = text..'#phone : +'..(msg.from.phone or '')..'\n'
  ...
  user_info_msgs = tonumber(redis:get(um_hash) or 0)
     ...
     text = text..'«?œ? ê—ÊÂ : '..msg.to.id
    end
	text = text..'\n\n @PersianGulf_Team: ?„ Å—‘?‰ êÊ·›'
