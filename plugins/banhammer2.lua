...
--vardump(extra)
      ...
      local receiver = "chat#id"..chat_id
       if get_cmd == "kick" then
         ...
         if is_momod2(member_id, chat_id) and not is_admin2(sender) then
            return send_large_msg(receiver, "شما نمیتوانید مدیران و ادمین هارو اخراج کنید.")
         end
         return kick_user(member_id, chat_id)
      elseif get_cmd == 'ban' then
        if is_momod2(member_id, chat_id) and not is_admin2(sender) then
          return send_large_msg(receiver, "شما نمیتوانید مدیران و ادمین هارو بن کنید.")
        end
      ...
      elseif get_cmd == 'unbanall' then
        send_large_msg(receiver, 'کاربر @'..member..' ['..member_id..'] از بن گلوبال خارج شد.')
...
local function run(msg, matches)
 if matches[1]:lower() == 'id' then
  ...
  if matches[1]:lower() == "banlist" then -- لیست بن... !
    local chat_id = msg.to.id
    ...
    if type(msg.reply_id)~="nil" and is_momod(msg) then
      ...
      if string.match(matches[2], '^%d+$') then
        ...
        if not is_admin(msg) and is_momod2(matches[2], msg.to.id) then
          	return "شما نمیتوانید مدیران و ادمین هارو بن کنید."
        end
        if tonumber(matches[2]) == tonumber(msg.from.id) then
          	return "شما نمتوانید خودتان را بن کنید !"
        end
        local name = user_print_name(msg.from)
...
if matches[1]:lower() == 'sik' then
	...
	if string.match(matches[2], '^%d+$') then
		...
		if not is_admin(msg) and is_momod2(matches[2], msg.to.id) then
			return "شما نمیتوانید مدیران و ادمین هارو اخراج کنید."
		end
		if tonumber(matches[2]) == tonumber(msg.from.id) then
    ...
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then 
        	...
        	return 'کاربر ['..user_id..' ] از لیست بن گلوبال حذف شد.'
      else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'unbanall',
  ...
  if matches[1]:lower() == "gblist" then -- لیست بن گلوبال...!
    return banall_list()
  end
end
